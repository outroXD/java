...
## Puzzle 71
* スコープ内にあるメンバーは、staticインポートよりも優先される。

## Puzzle 72
* final修飾子はメソッド、フィールドに付与されるのでは異なる意味合いになる。
* メソッドに対するfinal
  * オーバーライドの禁止。
  * staticメソッドでは隠蔽できない。
* フィールドに対するfainal
  * そのフィールドに1回しか代入できない。
* フィールドを隠蔽することはよくない書き方。
  * 包摂を破り、型とその型のメンバー間の関係に関して読みづらくなる。

## Puzzle 83
* DogはExceptionを継承していて、ExceptionはSerializableを継承している。
* Serializableを実装しているシングルトンクラスは、その唯一のインスタンスを返すreadResolveメソッドを持たなければならない。
* Serializable
  * バイト列に変換可能
  * C言語ではメモリ上に直列に並べられるので、そのメモリ上の値をファイルシステムにそのまま保存することで、構造体なんかをファイルに保存できる。
  * JavaではSerializableを指定されたクラスのみがバイト列に変換可能になる。
    * = インスタンスをファイルに保存・復元できる。
  * https://qiita.com/NBT/items/9f76c9fd1c7a90506658

## Puzzle 85
* クラスの初期化中にバックグラウンドスレッドを待つような実装は作らない。デッドロックになる可能性がある。
